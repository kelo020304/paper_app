...
function analysisHtml(paper, analysis, hasImage) {
  ...
  <title>${escapeHtml(paper.title)} — Paper Orbit</title><style>
  ...
  <h2>Related Works 表述</h2><p>${escapeHtml(analysis.relatedWorks)}</p><div class="footer">由 Paper Orbit AI 生成，请以论文原文为准。</div></body></html>`;
}
...
async function fetchPaperHtml(paper) {
  ...
  for (const url of urls) {
    try {
      const response = await fetch(url, { headers: { 'User-Agent': 'PaperOrbit/0.4 (personal research library)' } });
